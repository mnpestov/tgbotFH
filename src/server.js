...
const { setPaidByProviderOrderId, setFailedByProviderOrderId, getByProviderOrderId } = require('./services/orderService');
const { verifySignature } = require('./services/providers/sbpMock');
const { formatRub } = require('./services/payment');
...
app.post('/provider/webhook', async (req, res) => {
  try {
    ...
    if (event === 'paid') {
      await setPaidByProviderOrderId(providerOrderId);
      const order = await getByProviderOrderId(providerOrderId);
      logger.info({ providerOrderId }, 'order paid');

      if (order?.chatId) {
        const text = `✅ Оплата получена\nТариф: *${order.tariffCode}*\nСумма: *${formatRub(order.amountKopecks)}*`;
        try {
          await bot.telegram.sendMessage(order.chatId, text, { parse_mode: 'Markdown' });
          logger.info({ chatId: order.chatId }, 'notify user: sent');
        } catch (e) {
          logger.error({ chatId: order.chatId, err: e?.response || e }, 'notify user: failed');
        }
      } else {
        logger.warn({ providerOrderId }, 'notify user: missing chatId');
      }
    } else if (event === 'failed') {
      await setFailedByProviderOrderId(providerOrderId);
      const order = await getByProviderOrderId(providerOrderId);
